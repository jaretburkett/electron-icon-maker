...
args
  ...
  .option('silent', `Don't log anything beside errors`);

const flags = args.parse(process.argv);

function logConsole(...messages) {
  if (!flags.silent) {
        ...
        .then(results => {
          icongen(PNGoutputDir, oSub + 'win/', {
            ...
            .then(results => {
              ...
              logConsole('Renaming PNGs to Electron Format');
              renamePNGs(0);
            })
            .catch(err => {
              if (err) throw new Error(err);
...
function renamePNGs(position) {
  ...
  fs.rename(PNGoutputDir + startName, PNGoutputDir + endName, err => {
    logConsole('Renamed ' + startName + ' to ' + endName);
    if (err) {
      throw err;
    } else if (position < pngSizes.length - 1) {
